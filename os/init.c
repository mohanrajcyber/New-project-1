...
static void print_banner(void)
{
    ...
    fputs("  root     : initramfs + /data persist when AAHA_DISK is set\n",
          stdout);
    fputs("  login    : console; dropbear on Net/Lab (blank password)\n", stdout);
    fputs("\n", stdout);
    print_file("/etc/motd");
...
int main(int argc, char **argv)
{
    ...
    if (access("/usr/lib/aaha/bringup.sh", X_OK) == 0 ||
        access("/usr/lib/aaha/bringup.sh", R_OK) == 0) {
        pid_t br = fork();
        if (br == 0) {
            execl("/bin/sh", "sh", "/usr/lib/aaha/bringup.sh", (char *)NULL);
            _exit(127);
        }
        if (br > 0) {
            int st = 0;
            while (waitpid(br, &st, 0) < 0 && errno == EINTR) {
            }
        }
    }
